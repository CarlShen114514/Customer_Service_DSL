...
async def login():
    ...
    ], validate=lambda d: ('usr', '用户名不能为空') if d['cmd'] == '发送' and not d['usr'] else None)
    if username is None:
        run_js('window.close()') 
    if username['cmd'] == '注册':
        await registration()
    ...
    ], validate=lambda d: ('pwd', '密码不能为空') if d['cmd'] == 'Send' and not d['pwd'] else None)
    if password is None:
        run_js('window.close()') 
    try:
...
async def chat_page():
    ...
    for msg in hello_info:
        hello_msgs.append(('客服', msg))
    while True:
        data = await input_group(('发送消息'), [
            input(name='msg'),
            actions(name='cmd', buttons=['发送', {'label': '退出', 'type': 'cancel'}])
        ], validate=lambda d: ('msg', '消息不能为空') if d['cmd'] == 'Send' and not d['msg'] else None)
        if data is None:
            break
